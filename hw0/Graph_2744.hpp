...
public:	
  ...
  struct IndexTupil {
    size_type idx1, idx2;
    IndexTupil() { }
    IndexTupil(size_type index1, size_type index2):
 ...
 public:
  ...
  class Node {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
      return graph_ptr->point_set[idx];
    }
    ...
    bool operator==(const Node& n) const {
      // HW0: YOUR CODE HERE
      return (graph_ptr == n.graph()) && (idx == n.index());
    }
  ...
  bool has_node(const Node& n) const {
    // HW0: YOUR CODE HERE
    if (n.graph() != this || n.index() >= node_num)
      return false;
    else
      return true;
  }
  ...
  Edge edge(size_type i) const {
    // HW0: YOUR CODE HERE
    assert(i < edge_num);
    return Edge(this, edge_list[i].idx1, edge_list[i].idx2, i);
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    const size_type start_idx = std::min(a.index(), b.index()),
      end_idx = std::max(a.index(), b.index());
    // start_idx: index of the starting node, the relatively smaller one
    // end_idx: index of the starting node, the relatively larger one
    const std::vector<IndexTupil>& a_neighbor = adjacency_list[start_idx];
    for (size_type i=0; i < a_neighbor.size(); ++i)
      if (a_neighbor[i].idx1 == end_idx)
        return true;
    return false;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    const std::vector<IndexTupil>& a_neighbor = adjacency_list[start_idx];
    for (size_type i=0; i < a_neighbor.size(); ++i)
      if (a_neighbor[i].idx1 == end_idx)           // the edge to be added exists
        return Edge(this, start_idx, end_idx, a_neighbor[i].idx2);
    // add edge info to the adjacency list
    adjacency_list[start_idx]. push_back( IndexTupil(end_idx, edge_num) );
    // add edge info to the edge index tupil list
    edge_list. push_back( IndexTupil(start_idx, end_idx) );
    return Edge(this, start_idx, end_idx, edge_num++);
  }

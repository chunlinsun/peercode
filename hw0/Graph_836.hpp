...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    ++num_nodes_;
    internal_node new_internal_node = internal_node(position, new_node_id);
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    std::vector<size_type> b_neighbors = adj_list_[b.node_id_];
    for (auto j = b_neighbors.begin(); j != b_neighbors.end(); ++j) {
      if (*j == a.node_id_) {
        return true;
      }
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    return Edge(this, a.node_id_, b.node_id_);        // Invalid Edge
  }
 ...
 private:
  ...
  struct internal_node {
    Point position;
    size_type node_id; 
    internal_node(const Point& position_, const size_type node_id_)
      : position(position_), node_id(node_id_) {}
  };

...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    size_ ++;
    return Node(this, size_ - 1);        // return new node
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for(size_type i = 0; i < edge_size_; i++){
      if(edge_elements_[i]->uid1 == a.index() && edge_elements_[i]->uid2 == b.index()){
        return true;
      }
      if(edge_elements_[i]->uid2 == a.index() && edge_elements_[i]->uid1 == b.index()){
        return true;
      }
    }
    return false;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
    // try to find the edge first
    for(size_type i = 0; i < edge_size_; i++){
      if(edge_elements_[i]->uid1 == a.index() && edge_elements_[i]->uid2 == b.index()){
        return Edge(this, i);
      }
      if(edge_elements_[i]->uid2 == a.index() && edge_elements_[i]->uid1 == b.index()){
        edge_elements_[i]->uid1 = a.index();
        edge_elements_[i]->uid2 = b.index();
        return Edge(this, i);
      }
    }
    // not found, add edge

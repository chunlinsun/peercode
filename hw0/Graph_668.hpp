...
class Graph{
 ...
 public:
  ...
  Node add_node(const Point& position) {
    internal_point my_point(position, size_);
  ...
  Node node(size_type i) const {
    if ((i >= 0) && (i < size_)) return Node(this, i);
    // HW0: YOUR CODE HERE
  ...
  class Edge {
   public:
    ...
    bool operator==(const Edge& e) const {
      if (this->graph_ == e.graph_ && this->eid_ == e.eid_){
          return true;
      }
      //HW0: YOUR CODE HERE
      return false;
   ...
   private:
    ...
    size_type nd1_;
    size_type nd2_;
    Edge(const Graph* graph, size_type eid, size_type nd1, size_type nd2){
        graph_ = const_cast<Graph*>(graph);
        eid_ = eid;
        nd1_ = nd1;
        nd2_ = nd2;
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    if (!(a == b) && a.graph_ == this && b.graph_ == this){
          for (size_type i = 0; i < num_edges_; i++){
              if (edges[i][0] == a.index() && edges[i][1] == b.index( ))
                  return true;
              if (edges[i][0] == b.index() && edges[i][1] == a.index( ))
                  return true;
          }
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (!(a == b) && a.graph_ == this && b.graph_ == this){
         ...
         return Edge(this, num_edges_ - 1, a.index(), b.index());
    }
    // HW0: YOUR CODE HERE
    return Edge();        // Invalid Edge
  }
 ...
 private:
    struct internal_point{
        Point position;
        size_type pid;
        internal_point(const Point& position, size_type pid){

...
class Graph {
 ...
 public:
  ...
  class Node {
   public:
    ...
    Node() {
      // HW0: YOUR CODE HERE
      //I guess here there is nothing to do
    }
     ...
     * @We assume the 2 nodes lies in the same graph
     */
  ...
  Node add_node(const Point& position) {
    ...
    nodes_vector.push_back(Internal_node(new_id,position));
  ...
  class Edge {
   public:
    ...
    Node node1() const {
      // HW0: YOUR CODE HERE
      return Node(graph_, graph_->edges_vector[uid_].nuid1);
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      return Node(graph_, graph_->edges_vector[uid_].nuid2);
    }
   ...
   private:
    ...
    Graph* graph_ ; 
    //The id defining the edge.
    size_type uid_;

    // A private constructor for Edge class. 
    Edge(const Graph* node_graph, size_type uid)
     : graph_(const_cast<Graph*>(node_graph)), uid_(uid){
     }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for(size_type l = 0; l < edges_vector.size(); l++){
        if ((edges_vector[l].nuid1 == a.uid_) and (edges_vector[l].nuid2 == b.uid_))
          {return true;}
        if ((edges_vector[l].nuid2 == a.uid_) and (edges_vector[l].nuid1 == b.uid_))
          {return true;}
    }
    return false;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
    size_type new_id = this -> edges_vector.size();
    edges_vector.push_back(Internal_edge(new_id, a.uid_, b.uid_));
 ...
 private:
  ...
  struct Internal_node {
    ...
    Internal_node(size_type uid,Point pos) 
      :uid(uid),pos(pos) {}
   ...
   struct Internal_edge {
    //The id defining the node
    size_type uid; //The id of the node
    size_type nuid1; //The position of our node
    size_type nuid2; //The position of our node

    Internal_edge(size_type uid,size_type nuid1, size_type nuid2) 
      :uid(uid),nuid1(nuid1),nuid2(nuid2) {}

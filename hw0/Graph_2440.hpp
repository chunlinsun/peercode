 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   ...
   private:
	...
	Point position_;
	/** Private Constructor */
	Node(const graph_type* graph, size_type index, Point position)
		: graph_(const_cast<graph_type*>(graph)),  index_(index), position_(position){
	}
  };
  ...
  Node add_node(const Point& position, const node_value_type& = node_value_type()) {
	...
	++size_;
	++next_node_idx_;
    return Node(this, next_node_idx_-1,   position);     
  }
  ...
  bool has_node(const Node& n)  const{
	...
	if (internal_nodes_.find(n.index_) != internal_nodes_.end() and
		(internal_nodes_.find(n.index_)->second).x == n.position_ .x and
		(internal_nodes_.find(n.index_)->second).y == n.position_.y and
		(internal_nodes_.find(n.index_)->second).z == n.position_.z)
				return true;
	else
		return false;
  }
  ...
  Node node(size_type i) const{
    // HW0: YOUR CODE HERE
	assert(i < size());
	Node result_node = Node(this, i, (internal_nodes_.find(i))->second);
  ...
  class Edge : private totally_ordered<Edge> {
	...
	*/
    Node node1() const {
      // HW0: YOUR CODE HERE
	  size_type n_ind;
	  if (std::get<0>(nodes_ind_) < std::get<1>(nodes_ind_))
		  n_ind = std::get<0>(nodes_ind_) ;
	  else
		  n_ind = std::get<1>(nodes_ind_);

 ...
 public:
  ...
  class Edge {
   public:
    ...
    Node node1() const {
      return graph_->node(graph_->edges_[uid_].first);   
    }

    /** Return the other node of this Edge */
    Node node2() const {
      return graph_->node(graph_->edges_[uid_].second);
    }
    ...
    bool operator==(const Edge& e) const {
      return (this->graph_ == e.graph_) && (this->uid_ == e.uid_);
    }
     ...
     */
    bool operator<(const Edge& e) const {
      return (this->uid_ < e.uid_) || ( (this->uid_ == e.uid_) && (ptr_smaller{}(this->graph_, e.graph_)));
    }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    pair_type c = pair_type(min,max);
    for (pair_type p : edges_){
      if (p==c) return true;
    }
    return false;
  }

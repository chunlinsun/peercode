...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    return Node(this, nodes_vector.size()-1);        // Invalid node
  }
  ...
  class Edge {
   public:
    ...
    Edge(const graph_type* graph_pointer_, const size_type* index_node1_pointer_, const size_type* index_node2_pointer_, size_type edge_index_) {
	  graph_pointer = graph_pointer_;
   ...
   private:
	...
	const size_type* index_node1_pointer;
	const size_type* index_node2_pointer;
  ...
  Edge edge(size_type i) const {
	...
	if ( edges_map.find(i) != edges_map.end() )
	  return Edge(this ,&edges_map.at(i)[0], &edges_map.at(i)[1], i);
	else
  ...
  bool has_edge(const Node& a, const Node& b) const {
	...
	if ( nodes_connections_map.find(b.node_index) != nodes_connections_map.end() )
		if ( nodes_connections_map.at(b.node_index).find(a.node_index) != nodes_connections_map.at(b.node_index).end() )
			doesit = true;
  ...
  Edge add_edge(const Node& a, const Node& b) {
	...
	if (has_edge(a, b))
      ...
      return Edge(this, &edges_map[nodes_connections_map[b.node_index][a.node_index]][0], &edges_map[nodes_connections_map[b.node_index][a.node_index]][1], nodes_connections_map[b.node_index][a.node_index]);
    else{
 ...
 private:
  ...
  // keys are node indeces, and value is a map where the keys are the indeces connected to the node

...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    // HW0: YOUR CODE HERE
    vec_node.push_back(position);      // Quiet compiler warning
  ...
  class Edge {
   public:
     ...
     */
    bool operator<(const Edge& e) const {           // Quiet compiler warning
      return (e.gr==gr)&&(e.nedge>nedge);
    }
   ...
   Edge(const Graph* graph, Node node1,Node node2,size_type nid)
    : gr(const_cast<Graph*>(graph)), n1(node1), n2(node2),nedge(nid) {}

 ...
 public:
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for(size_type i=0;i<num_edges();i++){
      if(smaller==edge_small[i] and bigger==edge_big[i]){
        return true;
      }
    }
    return false;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    for(size_type i=0;i<num_edges();i++){
      if(s_node==edge_small[i] and b_node==edge_big[i]){
        return Edge(this,s_node,b_node,i);
      }
    }
    edge_small.push_back(s_node);
    edge_big.push_back(b_node);

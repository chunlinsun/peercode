...
class Graph {
 ...
 public:
  ...
  class Node {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
      // dereference the graph and access the point at some index
      const Point* a = graph_->allpoints_.at(index_);
      return *a;
    }
    ...
    bool operator==(const Node& n) const {
      ...
      if (this->index_ != n.index()) return false; 
      return true;
    }
    ...
    bool operator<(const Node& n) const {
      // HW0: YOUR CODE HERE
      if (this->index_ < n.index()) return true;
      return false;
    }
  ...
  Node add_node(const Point& position) {
    // HW0: YOUR CODE HERE
    // allocate memory for position and add pointer to vector
    const Point* newpoint = new Point(position.x, position.y, position.z);
    this->allpoints_.push_back(newpoint);
  ...
  class Edge {
   public:
    ...
    Edge() : graph_(nullptr), index_(100), edgeinfo_() {
    ...
    Node node1() const {
      // HW0: YOUR CODE HERE
      size_type node_index = this->edgeinfo_.at(0);
      return Node(this->graph_, node_index);
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      size_type node_index = this->edgeinfo_.at(1);
      return Node(this->graph_, node_index);      // Invalid Node
    }
   ...
   private:
    ...
    // index
    size_type index_;
    // vector[node1 index, node2 index]
    std::vector<size_type> edgeinfo_;

    Edge(const graph_type* graph, size_type index, const Node& a, const Node& b) 
        : graph_(const_cast<graph_type*>(graph)), index_(index), edgeinfo_() {
	edgeinfo_.push_back(a.index());
	edgeinfo_.push_back(b.index());
	}
  };
  ...
  Edge edge(size_type i) const {
    ...
    return alledges_.at(i);        // Invalid Edge
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    // create an edge with a placeholder index of 0
    Edge e = Edge(this, 0, a, b);

    // search through our vector of edges
    unsigned counter = 0;
    while (counter < numedges_) {
      if (alledges_.at(counter) == e) return true;
      counter++;}
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (a == b) {
      std::cout << "Identical nodes" << std::endl;
      return Edge();}
    ...
    // case where edge exists, return with invalid index

    return Edge(this, 0, a, b);
  }
 ...
 private:
  ...
  std::vector<const Point*> allpoints_;
  // the size of the graph (number of nodes)
  unsigned size_;
  // a vector to hold all the edges

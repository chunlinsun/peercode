...
class Graph {
 ...
 public:
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    //by design, small index always comes first
    for(unsigned int i = 0; i < edges_.size(); i++){
      if (edges_[i][0] == small_idx){
        if (edges_[i][1]== big_idx){
          return true;
	}
      }
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    for(unsigned int i = 0; i < edges_.size(); i++){
      if (edges_[i][0]==small_idx){
	if ( edges_[i][1]==big_idx){
	  return Edge(this, small_idx, big_idx, i);
	}
      }
    } 
    //always put the small index as the first node,
    //and the big index as the second node.
    std::vector<size_type> temp = {small_idx,big_idx};

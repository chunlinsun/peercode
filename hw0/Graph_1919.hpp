...
//    return 0;
  ...
  Node add_node(const Point& position) {
    ...
    internalNode resultInternalNode = internalNode(position, result_node_idx);
    node_vec_.push_back(resultInternalNode);
    neighbors_.push_back(std::vector<size_type>());
//    assert(node_vec_.size()==neighbors_.size());
...
//    return Edge();        // Invalid Edge
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for (auto n : neighbors_[a.idx_]) {
      ...
      // check the neighbors of node b
    for (auto n : neighbors_[b.idx_]) {
        if (n == a.idx_) {
            return true;
        }
    }
...
//    return false;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
//    std::cout << "edge " << a.idx_ << ", " << b.idx_ << std::endl;
    ...
    if (has_edge(a, b)) {
        ...
        // only needs to update the neighbor of one node-end
 ...
 private:
  ...
  struct internalNode {
      ...
      internalNode(const Point& position, const size_type idx):
        position_(position), idx_(idx) {

 ...
 Graph() :  node_idx(), edge_idx(), graph_nodes(), graph_edges(), next_nid(0), next_eid(0), node_size(0), edge_size(0)  { 
  ...
  class Node: private totally_ordered<Node>{
   ...
   node_value_type& value() {
     ...
     */
//--functionality_1
//--checking just the graph pointers is not enough. You should check index
//--START         
...
//--END
   ...
   */
//--functionality_1
//--need a add node function that takes a value
//--START    
...
//--END
     ...
     Node add_node(const Point& position, node_value_type nval)  {
...
//--design_1
//--warning generated if there is no guaranteed return (return only on if else). I've corrected it here
//--START 
    else {
...
NodeIterator(const Graph* graph, typename std::vector<internal_nodes>::const_iterator it_initial): graph_(const_cast<Graph*>(graph)), iter_node(it_initial) {}
  ...
  // node_iterator node_end() const
    ...
    /** Return the las iterator, by adding the increment of the address.*/
    node_iterator node_end() const {

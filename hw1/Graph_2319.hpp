...
class Graph {
 ...
 public:
  ...
  class Node  : private totally_ordered<Node> {
   public:
    ...
    size_type degree() const {
      if (g_->node_to_node_to_edge_.count(index())) {
        return g_->node_to_node_to_edge_[index()].size();
      }
    ...
    incident_iterator edge_begin() const {
      return IncidentIterator(g_,
                              g_->node_to_node_to_edge_[index()].begin());
    }
    ...
    return IncidentIterator(g_,
                              g_->node_to_node_to_edge_[index()].end());
    }
    ...
    bool operator==(const Node& n) const {
      // HW0: YOUR CODE HERE
      bool flag = false;
      if (n.g_ == this->g_ and n.index_ == index()) {flag = true;}
      return flag;
    }
    ...
    bool operator<(const Node& n) const {
      ...
      bool flag = false;
      if ((n.g_ == this->g_ and n.index_ < index()) or 
  ...
  bool has_node(const Node& n) const {
    // HW0: YOUR CODE HERE
    bool flag = false;
    if( n.g_ == this and n.index_ < num_nodes_) {flag = true;}
    return flag;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    size_type edge_ind;
    if (has_edge(a,b)) {edge_ind = node_to_node_to_edge_[a.index_][b.index_];}
    else{
      edge_ind = num_edges_;
  ...
  class EdgeIterator : private totally_ordered <EdgeIterator> { // inherit !=
   public:
     ...
     *
     * @pre ++ is called within a valid range of node's adjacency list.
     * @post result points to next valid edge in node's adjacency list, or
     ...
     */
//--functionality_1
//--After jumping to next inner map, need to continue iterating until you hit
//--an edge you haven't seen. Currently this EdgeIterator revisits some edges
//--and also can segfault, e.g. if it's finished with one node but the next node
//--has no incident edges.
//--START
    EdgeIterator& operator++() {
      size_type nn = graph_->num_nodes();
      size_type node1_id = nit_.ind_;
      
      // if at the last element (end minus one) of inner map
      if(init_.incd_it_ ==
         std::prev(graph_->node_to_node_to_edge_.at(node1_id).end())) {
        increment_helper(node1_id, nn);
      } // else can increment inner iterator, but must keep doing it till
        // either find node in adjacency list with node_id larger than node1_id
        // (to avoid revisiting edges) or hit the end of inner map.
      else {
        ++init_;
        while(init_ != graph_->node(node1_id).edge_end() and
              init_.incd_it_->first <= node1_id) {++init_;}
        if (init_ == graph_->node(node1_id).edge_end()) {
          increment_helper(node1_id, nn);
        }
      }
      return *this;
...
//--END
    ...
    bool operator==(const EdgeIterator& eit) const {
      return (eit.graph_ == this->graph_ and eit.nit_ == this->nit_ and 
              eit.init_ == this->init_);
    }
   ...
   private:
    ...
    const Graph* graph_;
    node_iterator nit_;
    incident_iterator init_;
    
    // Private constructor
    EdgeIterator(const Graph* graph, node_iterator nit, incident_iterator init)
                     : graph_(graph), nit_(nit), init_(init) {}

    // Helper function for increment
    void increment_helper(size_type& node1_id, const size_type nn) {
      if (node1_id == nn-1) { // if at the end of the outer map, set node and
                              // incident iterators to respective end()s.
        init_ = graph_->node(nn-1).edge_end();
        nit_ = graph_->node_end();
      }
      else { // else increment node iterator and set incident iterator to begin
             // corresponding to incremented node iterator.
        ++nit_;
        node1_id = nit_.ind_;
        init_ = graph_->node(node1_id).edge_begin();   
      }
    }
  };
  ...
  /** Return an edge iterator pointing to the first edge in the adjacency list
   * of the node with index 0 in the graph. */
  edge_iterator edge_begin() const {
    return EdgeIterator(this, node_begin(), node(0).edge_begin());
  }
  
  /** Return an edge iterator pointing past the last edge in the adjacency list
   * of the node with largest index in the graph. */

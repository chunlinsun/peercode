...
class Graph {
 private:
  ...
  // create an internal node with value and position
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    /** Return this node's position. */
     Point& position() {
      return graph_->nodes_[idx_].point;
      assert(false);
    }
    ...
    const node_value_type& value() const {
      return graph_->nodes_.at(idx_).value;
    }

//--functionality_1
//--Errors if node has degree 0
//--START
    /* Return the number of nodes that the current node is connected to*/
    size_type degree() const {
      return graph_->node_map_.at(idx_).size();
    }
//--END
    
    /* Return the beginning of the incident interator */
    incident_iterator edge_begin() const {
      return IncidentIterator(graph_, graph_->node_map_.at(idx_).begin(), idx_);
    }

    /* Return the end of the incident iterator */
    incident_iterator edge_end() const {
      return IncidentIterator(graph_, graph_->node_map_.at(idx_).end(), idx_);
    }
  ...
  class Edge: private totally_ordered<Edge> {
   public:
    ...
    edge_value_type& value() {
      return graph_->edges_[edge_idx_].value;
    }

    /* Return a read-only value of the node */
    const edge_value_type& value() const {
      return graph_->edges.at(edge_idx_).value;
    }
  ...
  Edge edge(size_type i) const {
    ...
    std::vector<size_type> e(edges_[i].nodes.begin(), edges_[i].nodes.end());
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    // adjacency list stores vectors <node b, edge e>, so index into 0 to
    // check if b exists
    std::vector<std::vector<size_type>> np = node_map_.at(a.index());
    for (std::size_t i = 0; i < np.size(); i++) {
      if (np.at(i).at(1) == b.index()) return true;
    }
   ...
   * @pre @idx is the current index of the of the edge
   * @return an Edge object e with e.node1() == @a a and e.node2() == @a b
   * @post add a node to the node adjacency list if it does not exist in the
   * current map, otherwise push a vector of <edge e, node b> into the vector
   * at node a
   */
  void add_edge_helper(size_type idx, const Node& a, const Node& b) {
    // if a vector at node a already exists, add a new vector
    if(node_map_.find(a.index()) == node_map_.end()) {
      std::vector<size_type> node_edge {idx, b.index()};
      std::vector<std::vector<size_type>> node_edge_set = {node_edge};
      node_map_[a.index()] = node_edge_set;
    }

    // otherwise push a vector of <edge e, node b> to node a
    else {
      std::vector<size_type> node_edge {idx, b.index()};
  ...
  Edge add_edge(const Node& a, const Node& b, const edge_value_type& value = edge_value_type()) {
    ...
    // node_map_ is a map from node a to a vector of <edge e, node b> vectors
    // iterate through node_map_ at a to get indices of already existing edges
//--functionality_1
//--idx is wrong here, probably want something like np[i][0] instead of i
//--START
    if (has_edge(a, b)) {
      size_type idx = 0;
      std::vector<std::vector<size_type>> np = node_map_.at(a.index());
...
//--END
    // create a new edge
      ...
      // a -> b and b -> a
      add_edge_helper(idx, a, b);
      add_edge_helper(idx, b, a);

      // push back a and b nodes as a set
      std::set<size_type> nodes = {a.index(), b.index()};
      internal_edge e(nodes, value);
      edges_.push_back(e);
...
//--END
   ...
   public:
    ...
    Edge operator*() const {
      ...
      std::vector<size_type> 
        e(graph_->edges_[idx_].nodes.begin(), graph_->edges_[idx_].nodes.end());
      return Edge(graph_, idx_, e[0], e[1]);
    }
 ...
 private:
  ...
  struct internal_node {
    ...
    internal_node(Point point, node_value_type value): 
  ...
  struct internal_edge {
    ...
    internal_edge(std::set<size_type> nodes, edge_value_type value) : 

...
private:
  ...
  //map of maps for Edges: first key is the "small" node, second key is 
  // the other node, and the value is the edge's number
  std::map<unsigned, std::map<unsigned, unsigned> > edges_;
  //edge counter because more efficient than traversing map of maps 8)
 ...
 public:
  ...
  Edge edge(size_type i) const {
    assert(i < num_edges());
    for (auto const &entry1 : edges_) {
      for (auto const &entry2 : entry1.second){
        if (entry2.second == i) {
          return Edge(this, entry2.second, entry1.first, entry2.first);
        }
      }
    }
    return Edge();
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    std::map<size_type, std::map<size_type, size_type> >::
      const_iterator iterator1 = edges_.find(searchValue1);
    if (iterator1 != edges_.end()) {
      std::map<size_type, size_type>::const_iterator iterator2 =
        iterator1->second.find(searchValue2);
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (iterator1 != edges_.end()) {
      std::map<size_type, size_type>::const_iterator iterator2 =
        iterator1->second.find(searchValue2);
      smallNodeWasThere = true;
      if (iterator2 != iterator1->second.end()){
        return Edge(this, iterator2->second, searchValue1, searchValue2);
      }
    }
    if(smallNodeWasThere){
      edges_[searchValue1].emplace(searchValue2, num_edges());
    } else {
      std::map<size_type, size_type> second_pair;
      second_pair.emplace(searchValue2, num_edges());
      edges_.emplace(searchValue1, second_pair);
    }
    edge_counter++;
    
    //THIS PART WAS AMBIGIOUS, SO I WROTE BOTH VERSIONS OF THE CODE
    //NOT CLEAR IF WE'RE SUPPOSED TO RETURN AN EDGE IF IT WAS ADDED.
    //MY CODE WORKS EITHER WAY!
    return Edge(this, num_edges(), searchValue1, searchValue2);
    //^ IF WE NEED TO RETURN THE NEWLY CREATED EDGE
    //return Edge();
    //^ IF WE DON'T NEED IT.
  }

 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    node_value_type& value() {return (graph_->allvalues_)[index_];}

    /** Return a non-modifiable reference to this node's value, const version of
    the value() function */
    const node_value_type& value() const {return (graph_->allvalues_).at(index_);}
    ...
    size_type degree() const {
      ...
      // degree = 0
      auto search = graph_->adjacency_.find(index_);
      if (search == graph_->adjacency_.end()) return 0;

      auto a_map = graph_->adjacency_.at(index_);
      return a_map.size();
    }
    ...
    bool operator<(const Node& n) const {
      // HW0: YOUR CODE HERE
      if (this->index_ < n.index_) return true;
      return false;
    }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    auto search = adjacency_.find(a.index());
    if (search == adjacency_.end()) return false;

    // find out if node is attached to b
    auto a_map   = adjacency_.at(a.index());
    auto search2 = a_map.find(b.index());
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (!(this->has_edge(a, b))) {
      ...
      adjacency_[a.index()][b.index()] = numedges_;
      adjacency_[b.index()][a.index()] = numedges_;
      numedges_++;
      return e;
    }

    // case where edge exists, return
    size_type edge_index = adjacency_[a.index()][b.index()];
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator> {
   public:
    ...
    Edge operator*() const {
      ...
      size_type node2    = mapitr_->first;
      size_type edge_ind = mapitr_->second;
   ...
   private:
    ...
    IncidentIterator(size_type counter, size_type node1, const graph_type* graph) : 
		     ...
		     Node nodea = Node(graph_, node1_);
		     if (nodea.degree() != 0) {mapitr_ = (graph_->adjacency_.at(node1_)).begin();}
		     }
  };
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator> {
   public:
    ...
    Edge operator*() const {
      ...
      return (graph_->alledges_).at(index_);
    }
...
//--documentation_0
//--passable documentation, please use more doxygen style next time
//--END 

...
class Graph {
   ...
   public:
    ...
    edge_value_type& value() {
      ...
      for (auto iter=adj_lst_tmp.begin(); iter!=adj_lst_tmp.end(); ++iter) {
        if (tmp) {
          if ((*iter).n2_index_ == n2_index_) return (*iter).e_val_;
        }
        else {
          if ((*iter).n2_index_ == n1_index_) return (*iter).e_val_;
        }
      }
  ...
  */
  struct Edge_Detail{
    ...
    Edge_Detail(size_type n1_index, size_type n2_index, edge_value_type e_val): \
                n1_index_(n1_index), n2_index_(n2_index), e_val_(e_val) {}
  };

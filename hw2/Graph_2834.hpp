...
class Graph {
 ...
 public:
  ...
  class Node: private totally_ordered<Node> {
   public:
    ...
    size_type degree() const{
      assert(valid());
      return graph_->nodes_[nid_].adj_.size();
    }
    ...
    incident_iterator edge_begin() const{
      assert(valid());
      return IncidentIterator(graph_, nid_, graph_->nodes_[nid_].adj_.begin());
    }
    ...
    incident_iterator edge_end() const{
      assert(valid());
      return IncidentIterator(graph_, nid_, graph_->nodes_[nid_].adj_.end());
    }
  ...
  Node add_node(const Point& position, const node_value_type& value = node_value_type()) {
    ...
    internal_node new_node(position, new_id, value, std::unordered_map<size_type, size_type>());
  ...
  size_type remove_node ( const Node & n) {
    ...
    if (has_node(n)) {
      ...
      // remove incident edges, take O(n.degree())
      while (nodes_[i2u_[n_idx]].adj_.size()>0) {
          auto eit = nodes_[i2u_[n_idx]].adj_.begin();
          size_type eid = eit->second;
          remove_edge(Edge(this,eid)); //each edge removal takes O(?//TODO)
      }
  ...
  node_iterator remove_node ( node_iterator n_it ){
    // call remove_node(const Node &), take O(num_nodes()) 
    remove_edge(*n_it);  
    return n_it;
  }
  ...
  class Edge: private totally_ordered<Edge> {
   public:
    ...
    Node node1() const {
      assert(valid());
      return Node(graph_,graph_->i2u_[graph_->edges_[graph_->i2e_[index()]].nid1]); 
    }

    /** Return the other node of this Edge */
    Node node2() const {
      assert(valid());
      return Node(graph_,graph_->i2u_[graph_->edges_[graph_->i2e_[index()]].nid2]); 
    }
  ...
  Edge add_edge(const Node& a, const Node& b, const edge_value_type e_value={}) {
    ...
    if (nodes_[i2u_[a.index()]].adj_.count(i2u_[b.index()])>0){ // unordered_map::count is O(1) for simple graph
      return Edge(this, nodes_[i2u_[a.index()]].adj_[i2u_[b.index()]]);
    }else{
      // create edge
  ...
  edge_iterator remove_edge ( edge_iterator e_it ){
    remove_edge(*e_it);  // call remove_edge(const Edge &), take O(1) average
    return e_it;
  }
  ...
  class IncidentIterator {
   public:
     ...
     */
    Edge operator*() const {
      return Edge(graphPtr_, adjIt_->second);
    }
    ...
    bool operator==(const IncidentIterator& incidentIter) const {
      return ((graphPtr_==incidentIter.graphPtr_) && (nid1_==incidentIter.nid1_) && (adjIt_==incidentIter.adjIt_));
    }
   ...
   private:
    ...
    std::unordered_map<size_type, size_type>::iterator adjIt_;
    /** Private Constructor */
    IncidentIterator(const graph_type* graph, const size_type nid1, std::unordered_map<size_type, size_type>::iterator adjIt): 
      graphPtr_(const_cast<graph_type*>(graph)), nid1_(nid1), adjIt_(adjIt) {}
 ...
 private:
  ...
  struct internal_node {
    ...
    std::unordered_map<size_type, size_type> adj_;  //set of <node id, corresponding edge id> for each neighboring node
    /** Constructor */
    internal_node(const Point &position, size_type node_index, node_value_type value, std::unordered_map<size_type, size_type> adj):
      pos(position), idx_(node_index), val(value), adj_(adj) {}
  };

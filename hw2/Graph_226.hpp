    ...
    */ 
    size_type remove_edge(const Node& n1, const Node& n2){
      if(has_edge(n1,n2)){
        ...
        unsigned counter=0;
        for(unsigned i=0;i<incidentmap[n1.intid()].size();i++){
          if(incidentmap[n1.intid()][i].index()==edgeid){
            std::iter_swap(incidentmap[n1.intid()].begin()+i,incidentmap[n1.intid()].begin()+incidentmap[n1.intid()].size()-counter-1);
            counter+=1;
          }
          if(counter==2){
            break;
        ...
        incidentmap[n1.intid()].pop_back();}
        if(incidentmap[n1.intid()].empty()){
          incidentmap.erase(n1.intid());
        }
        counter=0;
        for(unsigned i=0;i<incidentmap[n2.intid()].size();i++){
          if(incidentmap[n2.intid()][i].index()==edgeid){
            std::iter_swap(incidentmap[n2.intid()].begin()+i,incidentmap[n2.intid()].begin()+incidentmap[n2.intid()].size()-counter-1);
            counter+=1;
          }
          if(counter==2){
            break;

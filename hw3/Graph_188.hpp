...
class Graph {
   ...
   * @post Invalidates existing IncidentIterators
  ...
  class Edge: private totally_ordered<Edge> {
   ...
   private:
    ...
    size_type& edge_index(){
      auto it = graph_->elements_[node1().uid_].adj_.find(node2().uid_);
  ...
  bool has_edge(const Node& a, const Node& b) const {
    return elements_[a.uid_].adj_.find(b.uid_) != elements_[a.uid_].adj_.end();
  }
  ...
  Edge add_edge(const Node& a, const Node& b, const edge_value_type& value = edge_value_type() ) {
    if (! has_edge(a,b) ){
      ...
      elements_[a.uid_].adj_[b.uid_]=this->num_edges();
      elements_[b.uid_].adj_[a.uid_]=this->num_edges();
  ...
  size_type remove_edge(const Node& a, const Node& b){
    ...
    auto it = elements_[a.uid_].adj_.find(b.uid_);

    if (it == elements_[a.uid_].adj_.end())
      return 0;

    size_type index = (*it).second;

    elements_[a.uid_].adj_.erase(b.uid_);
    elements_[b.uid_].adj_.erase(a.uid_);

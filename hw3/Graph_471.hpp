 ...
 public:
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    bool operator==(const Edge& e) const {
      if (e.g == this->g && ((e.node1_idx_uuid == this->node1_idx_uuid && e.node2_idx_uuid == this->node2_idx_uuid) || 
        (e.node2_idx_uuid == this->node1_idx_uuid && e.node1_idx_uuid == this->node2_idx_uuid))) {
  ...
  Edge add_edge(const Node& a, const Node& b, const edge_value_type& value = edge_value_type()) {
    ...
    if (has_edge(a, b)) { //check to see if current edge already exists
      size_type node_1_uuid = node_i_to_u.at(a.ext_idx);
      size_type node_2_uuid = node_i_to_u.at(b.ext_idx);
      size_type edge_uuid = adj_map.at(node_1_uuid).at(node_2_uuid);

      Edge same_edge(node_i_to_u.at(a.ext_idx), node_i_to_u.at(b.ext_idx), internal_edges.at(edge_uuid).ext_idx, this);
      return same_edge;
    }
    else {
